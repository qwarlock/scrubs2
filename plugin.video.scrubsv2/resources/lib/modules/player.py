...
def playItem(url):
    try:
        ...
        control.infoDialog(
            "Error : No Stream Available. " + e.message, sound=False, icon="INFO"
        )
        return
...
class player(xbmc.Player):
    ...
    def run(self, title, year, season, episode, imdb, tmdb, tvdb, url, meta):
        try:
            ...
            self.year = year
            self.name = (
                urllib_parse.quote_plus(title) + urllib_parse.quote_plus(" (%s)" % year)
                if self.content == "movie"
                else urllib_parse.quote_plus(title)
                + urllib_parse.quote_plus(" S%01dE%01d" % (int(season), int(episode)))
            )
    ...
    def keepPlaybackAlive(self):
        ...
        if self.content == "movie":
            overlay = playcount.getMovieOverlay(
                playcount.getMovieIndicators(), self.imdb
            )
        ...
        if overlay == "7":
            ...
            while self.isPlayingVideo():
                try:
                    self.totalTime = self.getTotalTime()
                    self.currentTime = self.getTime()
                    watcher = self.currentTime / self.totalTime >= 0.92
    ...
    def idleForPlayback(self):
        for i in range(0, 400):
            if (
                control.condVisibility("Window.IsActive(busydialog)") == 1
                or control.condVisibility("Window.IsActive(busydialognocancel)") == 1
            ):
                control.idle()
            else:
    ...
    def onAVStarted(self):
        ...
        if (
            ...
            if control.setting("bookmarks.auto") == "true":
                ...
                if (
                    control.setting("bookmarks.source") == "1"
...
class subtitles:
    def get(self, name, imdb, season, episode):
        try:
            ...
            token = server.LogIn(un, pw, "en", "XBMC_Subtitles_Unofficial_v5.2.14")[
                "token"
            ]
            ...
            if control.setting("subtitles.notify") == "true":
                if xbmc.Player().isPlaying() and xbmc.Player().isPlayingVideo():
                    ...
                    control.infoDialog(
                        subname,
                        heading="{} subtitles downloaded".format(str(lang).upper()),
                        time=6000,
                    )
        except:
            pass
